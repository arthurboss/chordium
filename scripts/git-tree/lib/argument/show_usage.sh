...
show_usage() {
    ...
    echo "  --a               Auto mode: use all defaults, skip all prompts"
    echo "  --y               Auto-confirm cleanup of previous results (yes)"
    echo "  --n               Auto-decline cleanup of previous results (no)"
    ...
    echo "  $0 --a                               # Auto mode: all defaults, no prompts"
    ...
    echo "  $0 --y --base main                   # Auto-cleanup previous results + compare vs main"
    echo "  $0 --n --main --feat-search          # Skip cleanup + smart flags comparison"
