...
All static assets (favicons, PWA icons, app logo) live in `frontend/public/` and are served directly from the same origin as the app. See [`frontend/public/README.md`](../frontend/public/README.md) for a full file listing.
...
PWA icons, favicons, and the app logo are all in `frontend/public/`. The PWA manifest is generated by the VitePWA plugin and served from the same origin as the application.
