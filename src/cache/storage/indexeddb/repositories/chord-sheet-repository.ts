...
import { ChordSheetRecord, ChordSheetMetadata } from '../../../core/types';
import { ChordSheetDBConnection } from '../connection';
import { isChordSheetExpired } from '../../policies/retention-policy';
...
export class ChordSheetRepository {
  ...
  /**
   * Get all saved chord sheets with their paths (My Chord Sheets)
   */
  async getAllSavedWithPaths(): Promise<Array<{ path: string; chordSheet: ChordSheet }>> {
    const db = await this.connection.initialize();
    const transaction = db.transaction([CHORD_SHEET_DB_SCHEMA.stores.chordSheets.name], 'readonly');
    const store = transaction.objectStore(CHORD_SHEET_DB_SCHEMA.stores.chordSheets.name);
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const dbRecords = request.result as ChordSheetRecordDB[];
        // Convert number saved field back to boolean and extract chord sheets with paths
        const chordSheetsWithPaths = dbRecords.map(dbRecord => {
          const record = this.recordFromDB(dbRecord);
          return {
            path: record.path,
  ...
  /**
   * Delete a specific record by path (alias for delete)
   */
   ...
   */
  async updateTimestamp(path: string, timestamp: number): Promise<void> {
    const db = await this.connection.initialize();
    const transaction = db.transaction([CHORD_SHEET_DB_SCHEMA.stores.chordSheets.name], 'readwrite');
    const store = transaction.objectStore(CHORD_SHEET_DB_SCHEMA.stores.chordSheets.name);
    ...
    return new Promise((resolve, reject) => {
      ...
      getRequest.onsuccess = () => {
        ...
        if (dbRecord) {
          ...
          reject(new Error(`Record not found: ${path}`));
        }
      };
   ...
   * Update access count
   */
  private async updateAccessCount(path: string, accessCount: number): Promise<void> {
    const db = await this.connection.initialize();
    const transaction = db.transaction([CHORD_SHEET_DB_SCHEMA.stores.chordSheets.name], 'readwrite');
    const store = transaction.objectStore(CHORD_SHEET_DB_SCHEMA.stores.chordSheets.name);
