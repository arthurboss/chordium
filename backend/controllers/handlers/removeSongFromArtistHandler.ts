...
async function removeSongFromArtistHandler(
  ...
  try {
    ...
    logger.info(
      `Removing song with path "${songPath}" from artist "${normalizedArtistName}"`
    );
    ...
    if (success) {
      logger.info(
        `Successfully removed song with path "${songPath}" from artist "${normalizedArtistName}"`
      );
    ...
    res
      .status(500)
      .json({
        error: "Failed to remove song from artist",
