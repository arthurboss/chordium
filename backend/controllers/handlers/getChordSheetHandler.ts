...
async function getChordSheetHandler(
  ...
  try {
    ...
    if (!isValidChordSheetUrl(url)) {
      ...
      logger.error(
        `❌ URL must be a CifraClub URL with exactly 2 path segments (artist/song)`
      );
      res.status(400).json({
        error: "Invalid chord sheet URL",
        details:
          "URL must be a CifraClub chord sheet URL with format: artist/song",
      });
      return;
    }

    logger.info(
      `✅ Flow Step 1.5: URL validation passed - proceeding to scrape`
    );
    ...
    if (!chordSheet?.songChords) {
      logger.error(
        `❌ Flow Step 2: No chord sheet data returned from CifraClub service for ${url}`
      );
