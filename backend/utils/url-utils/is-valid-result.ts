...
export function isValidResult(
  ...
  try {
    ...
    if (!result || typeof result !== "object") {
      logger.debug(
        `URL validation failed: ${URL_VALIDATION_ERRORS.INVALID_RESULT}`
      );
      return false;
    }
    ...
    if (
      ...
      logger.debug(
        `URL validation failed: ${URL_VALIDATION_ERRORS.MISSING_URL}`,
        { result }
      );
      return false;
    }
    ...
    if (!searchType || typeof searchType !== "string") {
      logger.debug(
        `URL validation failed: ${URL_VALIDATION_ERRORS.INVALID_SEARCH_TYPE}`,
        { searchType }
      );
      return false;
    }
    ...
    try {
      ...
      logger.debug(
        `URL validation failed: ${URL_VALIDATION_ERRORS.INVALID_URL_FORMAT}`,
        { path, error: urlError }
      );
      return false;
    }
    ...
    logger.debug("[isValidResult] Normalized path and segments", {
      path,
      pathSegments,
    });
    ...
    if (
      ...
      (lastSegment.endsWith(".html") ||
        lastSegment.endsWith(".htm") ||
        lastSegment.includes("."))
    ) {
      logger.debug(
        `URL validation failed: ${URL_VALIDATION_ERRORS.INVALID_URL_FORMAT}`,
        { path, reason: "HTML or file extension detected" }
      );
      return false;
    }
    ...
    switch (searchType) {
      case SEARCH_TYPES.ARTIST:
        ...
        if (pathSegments.length !== 1) {
          logger.debug(
            `URL validation failed: ${URL_VALIDATION_ERRORS.INVALID_PATH_SEGMENTS}`,
            {
              searchType,
              pathSegments,
              expected: 1,
      ...
      case SEARCH_TYPES.SONG:
        ...
        if (pathSegments[1].toLowerCase() === "letra") {
          logger.debug(
            `Chord sheet URL validation failed: Last segment is 'letra'`,
            { path, pathSegments }
          );
          return false;
        }
