 ...
 */
export function normalizeArtistResults(
  artists: Artist[],
  ...
  return artists
    .map((artist) => {
      ...
      if (!displayName && (artist as any).title) {
        ...
        displayName = (artist as any).title
          .replace(/ - Cifra Club$/, "")
          .trim();
      }
 ...
 */
export function normalizeSongResults(
  songs: Song[],
  source: DataSource = "unknown"
): (Song & { displayName: string })[] {
