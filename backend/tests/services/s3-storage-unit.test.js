...
describe("S3StorageService Unit Tests", () => {
  ...
  describe("S3 Operations Error Handling", () => {
    ...
    test("should handle S3 NoSuchKey error correctly", async () => {
      ...
      const result = await s3StorageService.getArtistSongs(
        "non-existent-artist"
      );
  ...
  describe("Data Processing and Validation", () => {
    ...
    test("should generate correct S3 metadata", async () => {
      ...
      expect(putObjectCall.Metadata).toEqual({
        ...
        "song-count": "2",
        "last-updated": expect.stringMatching(
    ...
    test("should handle empty song arrays", async () => {
      ...
      const result = await s3StorageService.storeArtistSongs(
        "empty-artist",
        []
      );
    ...
    test("should validate song structure before adding", async () => {
      ...
      const result = await s3StorageService.addSongToArtist(
        "test-artist",
        newValidSong
      );
  ...
  describe("List Operations", () => {
    ...
    test("should handle S3 list operation failure", async () => {
      ...
      await expect(s3StorageService.listArtists()).rejects.toThrow(
  ...
  describe("Connection Testing", () => {
    ...
    test("should return false when S3 is disabled", async () => {
      process.env = {
        ...originalEnv,
        // Explicitly remove AWS credentials to disable S3
  ...
  describe("Key Generation and Formatting", () => {
    test("should generate correct S3 keys for artist songs", () => {
      const testCases = [
        ...
        {
          input: "artist-with-hyphens",
          expected: "artist-songs/artist-with-hyphens.json",
        },
        {
          input: "artist_with_underscores",
          expected: "artist-songs/artist_with_underscores.json",
        },
      ];
