...
describe('GET /api/cifraclub-search', () => {
  ...
  it('should search for artists with Supabase fallback', async () => {
    ...
    const response = await request(app).get('/api/cifraclub-search').query({ artist: 'test' });
  ...
  it('should search for songs directly with CifraClub', async () => {
    const mockSongs = fixtureLoader.getSongSearchResult('test').slice(0, 1);
    (mockCifraClubService.search as jest.Mock).mockImplementation(() => Promise.resolve(mockSongs));

    const response = await request(app).get('/api/cifraclub-search').query({ song: 'test' });
