...
describe('S3 Cache Performance Integration Tests', () => {
  ...
  describe('Cache vs Scraping Performance', () => {
    test('should demonstrate significant performance improvement with cache', async () => {
      ...
      mockCifraClubService.getArtistSongs.mockImplementation(() =>
        new Promise(resolve => setTimeout(() => resolve(mockSongs), 100))
      );
  ...
  describe('Concurrent Request Handling', () => {
    test('should handle multiple simultaneous cache requests efficiently', async () => {
      ...
      const createRequest = (artistPath) => ({
        query: { artistPath },
      });
      ...
      const requests = [
        searchController.getArtistSongs(createRequest('artist-1'), createResponse()),
        searchController.getArtistSongs(createRequest('artist-2'), createResponse()),
        searchController.getArtistSongs(createRequest('artist-3'), createResponse()),
        searchController.getArtistSongs(createRequest('artist-4'), createResponse()),
        searchController.getArtistSongs(createRequest('artist-5'), createResponse()),
      ];
    ...
    test('should handle mixed cache hit/miss scenarios', async () => {
      ...
      const createRequest = (artistPath) => ({ query: { artistPath } });
      const createResponse = () => ({
        status: jest.fn().mockReturnThis(),
        json: jest.fn().mockReturnThis(),
      });
      ...
      const requests = [
        searchController.getArtistSongs(createRequest('cached-1'), createResponse()),
        searchController.getArtistSongs(createRequest('uncached-1'), createResponse()),
        searchController.getArtistSongs(createRequest('cached-2'), createResponse()),
        searchController.getArtistSongs(createRequest('uncached-2'), createResponse()),
      ];
  ...
  describe('Cache Reliability and Error Recovery', () => {
    test('should gracefully degrade when S3 is completely unavailable', async () => {
      ...
      mockS3StorageService.getArtistSongs.mockRejectedValue(new Error('Service unavailable'));
      mockS3StorageService.storeArtistSongs.mockRejectedValue(new Error('Service unavailable'));
      mockCifraClubService.getArtistSongs.mockResolvedValue(mockSongs);
  ...
  describe('Memory and Resource Usage', () => {
    test('should not cause memory leaks with repeated cache operations', async () => {
      ...
      for (let i = 0; i < requestCount; i++) {
        requests.push(searchController.getArtistSongs(req, createResponse()));
      }
