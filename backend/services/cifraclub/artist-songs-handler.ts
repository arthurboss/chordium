 ...
 */
async function attemptPageLoad(
  page: Page,
  pageUrl: string,
  artistSlug: string
): Promise<Song[]> {
  ...
  for (const [index, strategy] of strategies.entries()) {
    try {
      logger.info(
        `Attempting artist page load with ${strategy.name} strategy (${index + 1}/${strategies.length})`
      );
      ...
      logger.info(
        `Artist page loaded with ${strategy.name} strategy, waiting ${strategy.waitAfter}ms for dynamic content...`
      );
      ...
      try {
        ...
        logger.debug(
          `Artist page loaded successfully with ${strategy.name} strategy`
        );
      } catch (selectorError) {
        const errorMessage =
        ...
        logger.warn(
          `Could not find body selector on ${pageUrl} with ${strategy.name} strategy:`,
          errorMessage
        );
      ...
      logger.info(
        `Successfully extracted ${songs.length} songs for ${artistSlug} using ${strategy.name} strategy`
      );
      return songs;
    } catch (error) {
      lastError = error instanceof Error ? error : new Error(String(error));
      logger.warn(
        `${strategy.name} strategy failed for ${artistSlug}:`,
        lastError.message
      );
  ...
  logger.error(
    `All loading strategies failed for ${artistSlug}. Last error:`,
    lastError.message
  );
  throw new Error(
    `Unable to load artist page for ${artistSlug}: ${lastError.message}. The page may be temporarily unavailable or blocked.`
  );
}
