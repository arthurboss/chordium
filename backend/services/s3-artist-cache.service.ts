...
async function storeArtistSongs(artistPath: string, songs: Song[]): Promise<boolean> {
  return await S3StorageService.storeArtistSongs(artistPath, songs);
}

async function addSongToArtist(artistName: string, song: Song): Promise<boolean> {
  return await S3StorageService.addSongToArtist(artistName, song);
}

async function removeSongFromArtist(artistName: string, songPath: string): Promise<boolean> {
