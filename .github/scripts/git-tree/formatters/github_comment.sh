...
format_for_github_comment() {
    ...
    # Create GitHub comment with proper markers
    {
        ...
        # Extract and output content directly (skip filepath comment, preserve original git-tree header)
        tail -n +2 "$git_tree_file" | grep -v "^<!-- filepath:"
        
        echo ""
        echo "---"
        
        # Generate absolute GitHub URL for the git-tree script
        local repo_url="${GITHUB_SERVER_URL:-https://github.com}/${GITHUB_REPOSITORY}"
        local git_tree_url="${repo_url}/tree/${GITHUB_BASE_REF}/scripts/git-tree"
        
        echo "_Generated by [git-tree script](${git_tree_url}) for PR #${PR_NUMBER}_"
