...
>(({ className, children, ...props }, ref) => {
  ...
  return (
    <TabsPrimitive.List
      ref={combinedRef}
      className={cn(
        "relative inline-flex h-12 items-center justify-center content-center rounded-md p-1.5 text-muted-foreground border bg-card",
        className
      )}
      {...props}
    >
      <span
        aria-hidden="true"
        className="absolute top-[0.375rem] bottom-[0.375rem] rounded-xs bg-background border transition-all duration-200 ease-in-out"
...
>(({ className, onKeyDown, ...props }, ref) => {
  ...
  return (
    <TabsPrimitive.Trigger
      ref={ref}
      className={cn(
        "relative inline-flex items-center justify-center whitespace-nowrap rounded-xs px-3 py-1.5 text-sm font-medium ring-offset-background transition-all focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 data-[state=active]:text-foreground",
        className
      )}
