 ...
 *
 * Reuses the existing artist-songs searchCache entry (see fetchArtistSongs)
...
export async function getStoredArtistDisplayName(artistPath: string): Promise<string | null> {
  try {
    ...
    return cached?.search.displayName ?? null;
  } catch {
    return null;
  }
}
  ...
  try {
    ...
    await searchCacheService.storeResults({
      ...
      search: {
        ...
        kind: "artist-songs",
        displayName,
        dataSource: cached?.search.dataSource ?? "cifraclub",
      },
    });
  } catch {
