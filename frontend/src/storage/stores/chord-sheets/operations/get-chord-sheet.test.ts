/**
 * Tests for getChordSheetContent's legacy-format auto-upgrade behavior.
 */
...
import { getChordSheetContent } from "./get-chord-sheet";
...
describe("getChordSheetContent", () => {
  ...
  it("returns ChordPro-format content unchanged and does not write back", async () => {
    const stored: StoredChordSheet = { path: "artist/song", songChords: "[G]Saying I love you" };
    mockExecuteReadTransaction.mockResolvedValueOnce(stored);

    const result = await getChordSheetContent("artist/song");
  ...
  it("returns null when no content exists for the path", async () => {
    ...
    const result = await getChordSheetContent("artist/missing-song");
